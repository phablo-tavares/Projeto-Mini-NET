...
MEU_MAC = "MAC_ROTEADOR"

tabela_roteamento = {
    "SERVIDOR PRIME": {"ip": "127.0.0.1", "port": 5000, "mac": "MAC_SERVIDOR"},
}
...
def iniciar_roteador():
    ...
    while True:
        try:
            ...
            if not integro:
                 print(f"{COR_VERMELHA}[ENLACE - ROTEADOR]{COR_RESET} Quadro corrompido fisicamente (Erro CRC). Descartando.")
                 continue
                 
            if quadro_dict.get('dst_mac') != MEU_MAC:
                 print(f"{COR_AMARELA}[ENLACE - ROTEADOR]{COR_RESET} Quadro ignorado. Destino não é este roteador.")
                 continue
            ...
            if src_vip and src_vip not in tabela_roteamento:
                if src_vip not in tabela_conhecidos or tabela_conhecidos[src_vip]["addr"] != endereco_remetente:
                    print(f"{COR_ROXA}[REDE - ROTEAMENTO]{COR_RESET} Atualizando rota para '{src_vip}' -> {endereco_remetente}")
                    tabela_conhecidos[src_vip] = {
                        "addr": endereco_remetente,
                        "mac": quadro_dict.get('src_mac')
                    }
            ...
            destino_info = tabela_roteamento.get(dst_vip)
            destino_real = None
            next_hop_mac = None
            
            if destino_info:
                destino_real = (destino_info["ip"], destino_info["port"])
                next_hop_mac = destino_info["mac"]
            elif dst_vip in tabela_conhecidos:
                destino_real = tabela_conhecidos[dst_vip]["addr"]
                next_hop_mac = tabela_conhecidos[dst_vip]["mac"]
            
            if destino_real and next_hop_mac:
                print(f"{COR_ROXA}[REDE - ROTEAMENTO]{COR_RESET} Encaminhando pacote para {destino_real[0]}:{destino_real[1]} (MAC: {next_hop_mac})")
                
                pacote_encaminhar = protocol.Pacote(src_vip, dst_vip, ttl, payload)
                
                quadro_encaminhar = protocol.Quadro(
                    src_mac=MEU_MAC,
                    dst_mac=next_hop_mac,
                    pacote_dict=pacote_encaminhar.to_dict()
                )
                
                bytes_encaminhar = quadro_encaminhar.serializar()
