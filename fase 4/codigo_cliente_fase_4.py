...
def iniciar_cliente():
    ...
    print(f"{COR_AZUL}="*50)
    print(f"   BEM-VINDO AO CHAT MINI-NET (FASE 4)   ")
    ...
    while True:
        try:
            ...
            quadro_enlace = protocol.Quadro(
                src_mac=MEU_MAC,
                dst_mac=ROUTER_MAC,
                pacote_dict=pacote_rede.to_dict()
            )
            
            quadro_bytes = quadro_enlace.serializar()
            ...
            while not ack_recebido:
                ...
                protocol.enviar_pela_rede_ruidosa(sock, quadro_bytes, (ROUTER_IP, ROUTER_PORT))
                
                try:
                    resposta_bytes, _ = sock.recvfrom(4096)
                    
                    quadro_dict, integro = protocol.Quadro.deserializar(resposta_bytes)
                    ...
                    if quadro_dict.get('dst_mac') != MEU_MAC:
                        print(f"{COR_AMARELA}[ENLACE]{COR_RESET} Quadro ignorado. MAC destino diferente do meu ({MEU_MAC}).")
                        continue
                         
                    pacote_resposta = quadro_dict.get('data', {})
